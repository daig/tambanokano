...
3. **The real prelude — `poly`/`Universal` + loading the actual library. ← IN PROGRESS: everything but the
   ...
   **Remaining prelude work, in dependency order:**
   - **(a) Container library — ✅ DONE (the data structures).** `EXT-BOOL`, `SET{Nat}`, `MAP{Nat,Nat}`,
     ...
     The parameterized container **views** parse (`view List{X :: TRIV} … to LIST{X}`), renamings over a
     **structured** sort parse (`* (sort NeList{Qid} to NeQidList)`), and the eq parser handles a `[_]`-list
     rhs / trailing `[owise]` — so the metalevel's simple container instantiations build & reduce
     byte-identically (`NAT-LIST`/`QID-LIST`/`QID-SET`) and the standalone `[_]`-list `LIST*`/`SET*` build
     (`conformance/view-parameterized.maude`, `eq-bracket-rhs.maude`). *Remaining:* **chained** multi-level
     instantiation `M{A}{B}` (the SORTABLE-LIST family) — it parses but `flatten` mangles the last-level sort
     substitution (the Axis-A5 residual on item 2 / `gaps.md`); off the data + `META-LEVEL` paths.
